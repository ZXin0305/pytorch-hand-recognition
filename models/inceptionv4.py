...
class InceptionV4(nn.Module):
    ...
    def __init__(self, in_ch, A, B, C, k=192, l=224, m=256, n=384, class_nums=100):

        super().__init__()
        self.stem = Inception_Stem(in_ch)
    ...
    def forward(self, x):
        x = self.stem(x)        # [1, 384, h, w] 
        x = self.inception_a(x) # [1, 384, h, w] 
        x = self.reduction_a(x) # [1, 1024, h, w]
        x = self.inception_b(x) # [1, 1024, h, w]
        x = self.reduction_b(x) # [1, 1536, h, w]
        x = self.inception_c(x) # [1, 1536, h, w]
        x = self.avgpool(x)     # [1, 1536, 1, 1]
        x = self.dropout(x)     # []
        x = x.view(-1, 1536)    # [1, 1536]
        x = self.linear(x)      # [1, num_classes]
...
def inceptionv4(in_ch):
    return InceptionV4(in_ch, 4, 7, 3)
...
if __name__ == "__main__":
    ...
    net1 = inceptionv4(in_ch = 4)
    net2 = inceptionv4(in_ch = 1)
    net1.to('cuda:0')
    net2.to('cuda:0')
    input1 = torch.ones(size=(1, 4, 160 ,160), dtype=torch.float32)
